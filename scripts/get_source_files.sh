...
find src/ -type f -name *.cpp -o -name *.h -o -name *.inl > cmake/SourceLists.txt
rg --replace "$(cat cmake/SourceLists.txt)" --passthru --no-line-number \
--multiline --multiline-dotall '__the_sources__' cmake/SourceLists.cmake.template | tee > cmake/SourceLists.cmake